<?php
function pagination( $limit_page){
	echo "Set results limit";
require_once 'conn.php';
$limited_results =$conn->prepare( "SELECT title, author, content, timestamp FROM posts LIMIT ?;");
$limited_results->bind_param("i", $limit_page);
$limited_results->execute();
$result = $limited_results->get_result();
...
function search_by($search_word,$filter_type){
require_once 'conn.php';
$search_query = "%" . $search_word . "%";
$allowed_filters = ['title',  'content'];
if (!in_array($filter_type, $allowed_filters)) {
    die("Invalid filter selected."); 
}
$query = "SELECT * FROM posts WHERE $filter_type LIKE ?;";
$by_filter = $conn->prepare($query);
